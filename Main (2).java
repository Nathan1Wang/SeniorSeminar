...
public class Main {
  //arraylists for main
  public static ArrayList<Student> stuList = new ArrayList<Student>(); //list(students)?
  public static ArrayList<Session> sesList = new ArrayList<Session>(); //list(sessions)?
  ...
  for (Student kid: stuList) {
    for (int i = 0; i<5; i++) {
      if (kid.getStuPref(i) == 1) {  //could you simplify this - by ref'ing thier choice?
        numSes1++;
      }
  ...
  //collection sort to sort the number of times a session is picked (cred to Arjun Gupta for the idea, code written by me and researched by me)// nice attribution!!
  ...
  //add additional sessions (sessions that are popular and need more than one run)
    //NATHAN = how did you know to add 7 sessions??
    for (int i = 0; i < 7; i++) {
  ...
  s1.loadStudents(stuList, sesList);
  System.out.println(sesList); //print rosters of sessions 
